...
class SiNE(nn.Module):
    def __init__(self, num_nodes, dim1, dim2):
        ...
        # third linear(end)
        self.layer3 = nn.Linear(dim2, 1, bias=False)
        self.bias3 = Parameter(torch.zeros(1))
        
        self.tanh = nn.Tanh()
        self.register_parameter('bias1', self.bias1)
        self.register_parameter('bias2', self.bias2)
        self.register_parameter('bias3', self.bias3)

    def forward(self, xi, xj, xk, delta, delta0):
        ...
        z21 = self.tanh(self.layer2(z11) + self.bias2)
        z22 = self.tanh(self.layer2(z12) + self.bias2)
    ...
    def _regularizer(self, x):
        ...
        normed = torch.norm(x - zeros, p=2)# turn to float
...
def get_training_batch(triples, batch_size):
    nrows = triples.shape[0]# read the size of first dimension
    rows = np.random.choice(nrows, batch_size, replace=False)
    choosen = triples[rows,:]# choose rows line in triplet dataset
